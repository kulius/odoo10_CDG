...
class WizardCashierDonor(models.Model):
    ...
    def consultant_block_num(self):
        if (self.block_num > 12):
            raise ValidationError(u'格子數不能超越13格')

        donate_data = self.env['normal.p'].search([('cashier_name','=',self.cashier_code.ids),('last_consultant_payment_date','>=',self.start_date),('last_consultant_payment_date','<=',self.end_date)]).ids
