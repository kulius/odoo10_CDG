...
class HouseYear(models.TransientModel):
    ...
    def call_print_report(self):
        donor_data = self.env['normal.p'].search([('merge_report','=',True)])
        datas = {
            'donor_data': donor_data.ids,
            'report_year': self.report_year,
            'key_in_user': self.key_in_user.name
        }
        return self.env['report'].get_action([], 'cdg_base.batch_house_year', datas)
