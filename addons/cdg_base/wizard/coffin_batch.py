        ...
        if self.flag == True:
            for data in self.coffin_data_lists:
                ...
                if data.finish == True:  # 判斷該專案是否已結案
                    ...
                    if data.donate_price == 0 and default_price_flag == True and data.exception_case == False:  # 沒有任何的捐助明細,而且確認申請金額是讀取基本設定檔的施棺滿足額, 而不是使用者自行輸入的特殊案例,
                        ...
                        if flag == False:  # 沒有單筆三萬元的捐款, 找大於1萬元的捐款
                            lines = self.env['donate.order'].search([('donate_type', '=', 3), ('available_balance', '>=', 10000),('use_amount', '=', False), ('donate_date', '<', data.coffin_date)])
                            num = len(lines)
                            if num >= 3:
                                for line in lines:
                                    if Cumulative_amount == 0:  # 達到施棺滿足額
                    ...
                    if data.donate_price != 0 or flag == False:  # 代表已有捐助資料 或 沒有單筆3萬元的資料
                        for line in data.batch_donate:  # 從捐助資料表中, 計算目前的累積金額
                            if int(float(line.donate_price)) >= 10000:
                                big_donate_flag = True
                        if big_donate_flag and data.exception_case == False:
                            lines = self.env['donate.order'].search([('donate_type', '=', 3), ('available_balance', '>=', 10000),('use_amount', '=', False), ('donate_date', '<', data.coffin_date)])
                            for line in lines:
                                if Cumulative_amount == 0:  # 達到施棺滿足額
                                    data.finish = True
                            ...
                            lines = self.env['donate.order'].search(['|', ('donate_type', '=', 3), ('donate_type', '=', 6),('available_balance', '<', 10000), ('use_amount', '=', False),('donate_date', '<', data.coffin_date)])
