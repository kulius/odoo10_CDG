...
class changestate(models.Model):
    ...
    end_donate_id = end_year = fields.Char(string='收據編號-訖')
    ...
    def change_state(self):
        datas = self.env['donate.single'].search([('donate_id','>=',self.star_donate_id),('donate_id','<=',self.end_donate_id),('key_in_user','=',self.key_in_user.id)])
        for line in datas:
            if line.state == 2:
                line.state = 1
        return True