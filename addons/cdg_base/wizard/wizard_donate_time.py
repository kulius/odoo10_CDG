...
class WizardCashierDonor(models.Model):
    ...
    def donate_time(self):
        ...
        data = self.env['donate.single'].search([('key_in_user','=',self.key_in_user_code.id),('donate_date','>=',self.donate_start_date),('donate_date','<=',self.donate_end_date),('work_id','=',self.cashier_code.id),('state','=',1)])
