    ...
    def generate_xlsx_report(self,workbook,data,env):
        ...
        for line in env:
            if not line:
                ...
                sheet.write(self.count, 2, line.fee_code)
                sheet.write(self.count, 3, line.normal_p_id.name)
                sheet.write(self.count, 4, line.fee_payable)
                sheet.write(self.count, 5, line.con_phone)
                sheet.write(self.count, 6, line.cellphone)
                sheet.write(self.count, 7, line.rec_addr)
                sheet.write(self.count, 8, line.clerk_id)
                self.count += 1
